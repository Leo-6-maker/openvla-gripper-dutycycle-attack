...
def test_happy_path_constructs_static_invocation(tmp_path):
    r = call(tmp_path, audit(tmp_path), runner(tmp_path, "--parent-id --episode-key --initial-state-hash --output-json"))
...
def test_fail_count(tmp_path):
    r = call(tmp_path, audit(tmp_path, dataset_match_count=2), runner(tmp_path, "--parent-id --episode-key --initial-state-hash --output-json"))
    assert r["status"] == "HOLD_MATCH_COUNT_NOT_UNIQUE"
    assert r["constructed_invocation"]["argv"] == []


def test_missing_reset_field(tmp_path):
    r = call(tmp_path, audit(tmp_path, reset_candidate_fields={}), runner(tmp_path, "--parent-id --episode-key --initial-state-hash --output-json"))
    assert r["status"] == "HOLD_RESET_FIELD_MISSING"


def test_unsupported_runner_arg(tmp_path):
    r = call(tmp_path, audit(tmp_path), runner(tmp_path, "--parent-id --episode-key --output-json"))
