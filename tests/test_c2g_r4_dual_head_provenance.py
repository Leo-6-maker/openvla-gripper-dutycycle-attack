...
class R4DualHeadProvenanceTests(unittest.TestCase):
    def fixture(self, root: Path):
        ...
        episode = collection / "episodes" / "libero_10" / "task_0" / "episode_000"
        ...
        collection_report_path = collection / "c2g_clean_collection_report.json"
            ...
            json.dumps(
                {
                    **canonical_common,
                    ...
                    "reason_code_counts": {"TARGET_CRITICAL_WINDOW_START": 1},
                    "violation_count": 0,
                },
                sort_keys=True,
            ),
        ...
        previous_canonical.write_text(
            json.dumps(
                {
                    **canonical_common,
                    "status": "HOLD_C2G_CLEAN_WINDOW_V2_DRY_AUDIT",
                    "attack_start_row_count": 2,
                    "reason_code_counts": {"TARGET_CRITICAL_WINDOW_START": 2},
                    "violation_count": 1,
                },
                sort_keys=True,
            ),
        ...
        goal.write_text(
            json.dumps(
                {
                    "status": "PASS_C2G_GOAL_EVENT_TRACKING_AUDIT",
                    "input_root": str(collection.resolve()),
                    "violation_count": 0,
                    "violations": [],
                    "totals": {**goal_totals, "attack_start_rows": 1},
        ...
        previous_goal.write_text(
            json.dumps(
                {
                    "status": "HOLD_C2G_GOAL_EVENT_TRACKING_AUDIT",
                    "input_root": str(collection.resolve()),
                    "violation_count": 1,
                    "violations": [
                        {
                    ...
                    "totals": {**goal_totals, "attack_start_rows": 2},
                    "uses_attack_outcomes": False,
                    "openvla_model_loads": 0,
                    "libero_environments_created": 0,
                    "attacks_launched": 0,
        ...
        previous_binding.write_text('{"status":"HOLD"}\n', encoding="utf-8")
        label_builder = root / "label_builder.py"
        label_builder.write_text("# frozen label builder\n", encoding="utf-8")
        return {
    ...
    def build(self, fixture):
        return build_binding(
            ...
            collection_report_path=fixture["collection_report"],
            collection_binding_report_path=fixture["collection_binding"],
            ...
            previous_canonical_hold_path=fixture["previous_canonical"],
            previous_goal_event_hold_path=fixture["previous_goal"],
            previous_hold_binding_path=fixture["previous_binding"],
        )
    ...
    def test_build_and_verify_one_shot_dual_head_binding(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(report["collection_head"], fixture["collection_head"])
            self.assertEqual(report["audit_head"], fixture["audit_head"])
            self.assertTrue(report["source_collection_unchanged"])
            self.assertEqual(report["unknown_to_negative_count"], 0)
            self.assertEqual(
                report["expected_only_change"]["libero_10_attack_start_rows"],
                "2_to_1",
            )
            binding_path = Path(td) / "binding.json"
    ...
    def test_verify_rejects_collection_mutation(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            report = self.build(fixture)
            binding_path = Path(td) / "binding.json"
            binding_path.write_text(json.dumps(report), encoding="utf-8")
            fixture["steps"].write_text('{"step": 1}\n', encoding="utf-8")
    ...
    def test_build_rejects_unexpected_label_drift(self):
        with tempfile.TemporaryDirectory() as td:
            fixture = self.fixture(Path(td))
            canonical = json.loads(fixture["canonical"].read_text(encoding="utf-8"))
            canonical["critical_positive_row_count"] += 1
            canonical["known_negative_row_count"] -= 1
            fixture["canonical"].write_text(json.dumps(canonical), encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "unexpected canonical label drift"):
                self.build(fixture)
