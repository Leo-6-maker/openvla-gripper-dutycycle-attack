#!/usr/bin/env python3
"""C2e2 v2 temporal pooling MLP training gate (val-only selection).

Fixes v1 test-set leakage: model/config/threshold selection uses validation
split only. Test split is evaluated once on the final selected model.
...
GATE = "C2E2_TEMPORAL_POOLING_MLP_TRAINING_V2"
PASS = "PASS_C2E2_TEMPORAL_POOLING_MLP_TRAINED"
HOLD = "HOLD_C2E2_TEMPORAL_POOLING_MLP_TRAINING"
WINDOWS_DEFAULT = [8, 16, 32]
OUT_FILES = [
    "c2e2_v2_temporal_pooling_mlp_report.json",
    "c2e2_v2_all_config_metrics.csv",
    "c2e2_v2_selected_threshold_sweep.csv",
    "c2e2_v2_selected_test_predictions.csv",
    "c2e2_v2_selected_test_metrics_by_suite.csv",
    "c2e2_v2_selected_model_config.json",
    "c2e2_v2_violations.csv",
...
def threshold_sweep(y: np.ndarray, logits: np.ndarray, suite: np.ndarray, tau_values: Sequence[float], min_recall: float, max_fp: float) -> List[Dict[str, Any]]:
    ...
    for tau_emit in tau_values:
        for tau_suppress in tau_values:
            ...
            score = m["f1"] + 0.5 * m["recall"] - 3.0 * max(0.0, min_recall - m["recall"]) - 3.0 * max(0.0, m["fp_rate"] - max_fp)
...
def evaluate_config(dataset: Dict[str, Any], model: PoolingMLP, min_val_recall: float, max_val_fp: float, tau_values: Sequence[float]) -> Dict[str, Any]:
    ...
    # === TEST: evaluate once, DO NOT use for selection ===
...
def _train_worker(args_dict: Dict[str, Any]) -> Dict[str, Any]:
    """Train one config in a worker process. Returns metrics dict."""
    c2e1_root = Path(args_dict["c2e1_root"])
    cfg = TrainConfig(**args_dict["config"])
    epochs = args_dict["epochs"]
    batch_size = args_dict["batch_size"]
    patience = args_dict["patience"]
    min_val_recall = args_dict["min_val_recall"]
    max_val_fp = args_dict["max_val_fp"]
    tau_values = args_dict["tau_values"]
    torch_threads = args_dict.get("torch_threads", 4)

    torch.set_num_threads(max(1, torch_threads))
    dataset = load_window_dataset(c2e1_root, cfg.window)
    model, train_info = train_one_config(dataset, cfg, epochs, batch_size, patience)
    ev = evaluate_config(dataset, model, min_val_recall, max_val_fp, tau_values)
    ...
    val_fp_rate = float(row["val_fp_rate"])
    selection_score = (
        val_f1 + 0.5 * val_recall
        - 3.0 * max(0.0, min_val_recall - val_recall)
        - 3.0 * max(0.0, val_fp_rate - max_val_fp)
    )
    feasible = bool(val_recall >= min_val_recall and val_fp_rate <= max_val_fp)
...
def main() -> int:
    ...
    ap.add_argument("--torch-threads", type=int, default=max(1, min(4, os.cpu_count() or 2)))
                ...
                for lr in lr_grid:
                    for seed in seed_grid:
                        all_configs.append({
                            ...
                            "config": {"window": window, "hidden": hidden, "dropout": dropout, "lr": lr, "weight_decay": args.weight_decay, "seed": seed},
                            ...
                            "tau_values": tau_values, "torch_threads": args.torch_threads,
                        })

    n_workers = min(args.workers, len(all_configs))
    print(f"C2e2 v2: {len(all_configs)} configs × {n_workers} workers, torch_threads={args.torch_threads}")

    all_config_rows: List[Dict[str, Any]] = []
    best_row: Optional[Dict[str, Any]] = None
    completed = 0

    # Parallel training across configs
    with ProcessPoolExecutor(max_workers=n_workers) as pool:
        futures = {pool.submit(_train_worker, cfg): cfg for cfg in all_configs}
        for fut in as_completed(futures):
            row = fut.result()
            all_config_rows.append(row)
            completed += 1

            # Track best by val metrics
            feasible = bool(row["feasible"])
            sel = float(row["selection_score"])
            key = (feasible, sel, float(row["val_f1"]), float(row["val_recall"]), -float(row["val_fp_rate"]))
            if best_row is None or key > best_row["_key"]:
                row["_key"] = key
                best_row = row

            if completed % 12 == 0 or completed == len(all_configs):
                print(f"  completed {completed}/{len(all_configs)} configs...")

    if best_row is None:
        raise RuntimeError("no training config completed")

    # Retrain best config in main process to get model object
    selected_cfg = TrainConfig(
        window=int(best_row["window"]), hidden=str(best_row["hidden"]),
        dropout=float(best_row["dropout"]), lr=float(best_row["lr"]),
        weight_decay=float(best_row["weight_decay"]), seed=int(best_row["seed"]),
    )
    torch.set_num_threads(max(1, args.torch_threads))
    selected_dataset = load_window_dataset(c2e1_root, selected_cfg.window)
    selected_model, train_info = train_one_config(selected_dataset, selected_cfg, args.epochs, args.batch_size, args.patience)
    selected_eval = evaluate_config(selected_dataset, selected_model, args.min_val_recall, args.max_val_fp, tau_values)
    selected_row = best_row
    test_mask = selected_dataset["split"] == "test"

    # === Test predictions (for final report only) ===
    ...
    # === Violations on TEST only (final gate) ===
    violations: List[str] = []
    ...
    # === Save model ===
    model_path = out / "c2e2_v2_selected_model.pt"
    torch.save({
    ...
    # === Outputs ===
    write_csv(out / "c2e2_v2_all_config_metrics.csv", all_config_rows, [
        "window", "hidden", "dropout", "lr", "weight_decay", "seed", "best_epoch", "epochs_run", "best_val_loss",
        "tau_emit", "tau_suppress",
        "val_tp", "val_fn", "val_fp", "val_tn", "val_recall", "val_fp_rate", "val_precision", "val_f1", "val_acc",
        "test_tp", "test_fn", "test_fp", "test_tn", "test_recall", "test_fp_rate", "test_precision", "test_f1", "test_acc",
        "test_suite_fp_max", "selection_score", "feasible",
    ])
    write_csv(out / "c2e2_v2_selected_threshold_sweep.csv", selected_eval["sweep"], [
        "tau_emit", "tau_suppress", "feasible", "score", "tp", "fn", "fp", "tn", "recall", "fp_rate", "precision", "f1", "acc",
    ])
    write_csv(out / "c2e2_v2_selected_test_predictions.csv", pred_rows, ["row_index", "suite", "split", "y", "emit_p", "suppress_p", "pred"])
    write_csv(out / "c2e2_v2_selected_test_metrics_by_suite.csv", selected_eval["suite_test_metrics"], [
        "split", "suite", "n", "tp", "fn", "fp", "tn", "recall", "fp_rate", "precision", "f1", "acc",
    ])
    write_json(out / "c2e2_v2_selected_model_config.json", {
    ...
    write_csv(out / "c2e2_v2_violations.csv", [{"violation": v} for v in violations], ["violation"])

    # Checksums
    checks = []
    for name in OUT_FILES + ["c2e2_v2_selected_model.pt"]:
        p = out / name
    ...
    report = {
        ...
        "boundaries": {
            ...
            "simulator_runtime": "NOT_PERFORMED",
    ...
    write_json(out / "c2e2_v2_temporal_pooling_mlp_report.json", report)
    ...
    print(json.dumps({
        ...
        "selected_config": asdict(selected_cfg),
        "selected_threshold": selected_eval["selected_threshold"],
        "val_recall": selected_row["val_recall"],
        "val_f1": selected_row["val_f1"],
