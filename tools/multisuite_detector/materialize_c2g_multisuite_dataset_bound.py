...
the dual-head R4 binding, and the four-suite model bytes before delegating to the
existing suite-isolated materializer.  It then emits a sidecar report that binds
the resulting dataset to the collection head, audit head, model manifests, and
materializer code bytes.

Use ``--dry-run`` for a CPU/read-only command preview.  No OpenVLA model is loaded
and no dataset is written in dry-run mode.
"""
...
from scripts.stageb.verify_c2g_suite_model_map_strict import verify as verify_models  # noqa: E402
from tools.multisuite_detector.bind_c2g_r4_dual_head_provenance import (  # noqa: E402
    verify_binding,
)

BASE_MATERIALIZER = REPO / "tools" / "multisuite_detector" / "materialize_c2g_multisuite_dataset.py"
...
def _assert_external_empty_output(output_dir: Path) -> None:
...
def preflight(args: argparse.Namespace) -> dict[str, Any]:
    ...
    _assert_external_empty_output(output_dir)
    ...
    if recorded_verification.get("suite_models") != model_verification.get("suite_models"):
        raise ValueError("recorded model verification differs from current model bytes")

    free_bytes = shutil.disk_usage(output_dir.parent if output_dir.parent.exists() else input_root).free
    ...
    return {
        ...
        "r4_provenance_binding_sha256": sha256_file(args.r4_provenance_binding.resolve()),
...
def _expected_combined_paths(args: argparse.Namespace) -> tuple[Path, Path]:
    output_dir = args.output_dir.resolve()
    dataset = output_dir / (
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    completed = subprocess.run(preflight_report["command"], cwd=REPO)
    if completed.returncode != 0:
        raise RuntimeError(f"base materializer failed with exit code {completed.returncode}")
    ...
    report = {
        ...
        "bound_wrapper_path": str(Path(__file__).resolve()),
        "bound_wrapper_sha256": sha256_file(Path(__file__).resolve()),
