...
GATE = "D1C_CLEAN2000_TEACHER_LABEL_V2_MANIFEST_MATERIALIZATION"
...
ANCHOR_FIELDS = ["teacher_anchor_step", "positive_anchor_step", "anchor_step", "event_step", "trigger_step", "target_step"]
WINDOW_START_FIELDS = ["teacher_window_start", "window_start", "positive_window_start", "event_window_start"]
WINDOW_END_FIELDS = ["teacher_window_end", "window_end", "positive_window_end", "event_window_end"]
...
def source_single_event_label(row: Dict[str, Any], args: argparse.Namespace) -> Tuple[Dict[str, Any] | None, Dict[str, Any] | None]:
    ...
    return {
        ...
        "label_source": "source_single_event_anchor_fields",
...
def libero10_label_rows(segment_rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for s in segment_rows:
        row = {
            ...
            "label_source": "d1a_libero10_segment_candidate_resolver",
            "online_input_allowed": "NO_LABEL_ONLY",
        }
        out.append(row)
    return out
...
def run(args: argparse.Namespace) -> int:
    ...
    if args.segment_quality_report:
        quality = json.loads(Path(args.segment_quality_report).read_text(encoding="utf-8"))
        quality_status = str(quality.get("status", ""))
        if quality_status != "PASS_LIBERO10_SEGMENT_CANDIDATE_QUALITY_AUDITED" and not args.allow_unverified_libero10_segments:
            raise SystemExit(f"segment quality report not PASS: {quality_status}")
    ...
    for row in source_rows:
        ...
        if suite in SINGLE_EVENT_SUITES:
            ...
            missing.append({"record_id": record_id(row), "suite": suite, "missing_reason": "UNKNOWN_SUITE", "available_columns": "", "source_file": row.get("__source_file", ""), "source_line": row.get("__source_line", "")})
    ...
    for suite in SUITES + ["UNKNOWN"]:
        ...
        if c or suite != "UNKNOWN":
            summary_rows.append({"suite": suite, "label_rows": c.get("label_rows", 0), "primary_rows": c.get("primary_rows", 0), "auxiliary_rows": c.get("auxiliary_rows", 0), "missing_records": c.get("missing_records", 0)})
    ...
    if len(source_rows) != args.expected_total:
        ...
        status = PASS
        reason = ""
    fields = ["record_id", "suite", "task_id", "task_name", "event_id", "segment_id", "segment_index", "segment_start_step", "segment_end_step", "teacher_anchor_step", "teacher_window_start", "teacher_window_end", "segment_role", "subtask_index", "event_role", "event_status", "phase_label", "corridor_label", "release_safe_label", "label_confidence", "label_source", "online_input_allowed"]
    write_csv(out / "clean2000_teacher_labels_v2_manifest.csv", labels, fields)
    write_csv(out / "clean2000_teacher_labels_v2_missing_single_event_sources.csv", missing, ["record_id", "suite", "missing_reason", "available_columns", "source_file", "source_line"])
...
def main() -> int:
    ...
    p.add_argument("--segment-quality-report", default="")
