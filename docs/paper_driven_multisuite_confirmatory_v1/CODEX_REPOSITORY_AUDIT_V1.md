...
| Streaming feature extraction | SYNTHETIC_CONTRACT_PASS_REAL_BINDING_PENDING | `tools/multisuite_detector/load_frozen_clean_features.py`; `detector_dataset_closure_v1.py` | `tests/multisuite_detector/test_detector_dataset_closure_v1.py` | C2 defines a strict synthetic frozen-feature artifact schema with exact 25D `SC5_FEATURES`, finite step features, feature schema SHA, artifact SHA, and required `initial_state_hash`. Real feature artifact binding and real `initial_state_hash` provenance remain PENDING_SCIENTIFIC_BINDING. |
