...
| Frozen feature artifact ingestion and Label V2 feature join | SYNTHETIC_CONTRACT_PASS_REAL_BINDING_PENDING |
| Parent split tooling | EXISTS_AND_REVIEWED_AFTER_C2_SYNTHETIC |
| State-hash leakage validation | SYNTHETIC_CONTRACT_PASS_REAL_BINDING_PENDING |
...
| Streaming feature extraction | SYNTHETIC_EXTRACTION_PATH_PASS_REAL_EXTRACTION_NOT_PERFORMED | `tools/multisuite_detector/extract_formal_25d_features_v1.py`; `validate_formal_25d_features_v1.py`; `audit_clean_rollout_feature_sources_v1.py`; `detector_dataset_closure_v1.py` | `tests/multisuite_detector/test_extract_formal_25d_features_v1.py`; `test_validate_formal_25d_features_v1.py`; `test_initial_state_hash_provenance_v1.py`; `test_detector_dataset_closure_v1.py` | C2-FX adds a synthetic-only formal 25D extraction path and validator. Real feature extraction remains NOT_PERFORMED; server binding decision remains `HOLD_NO_BINDABLE_ARTIFACT` until clean source records and `initial_state_hash` provenance are bound. |
| Exact-set episode joins | EXISTS_AND_REVIEWED_AFTER_C2_SYNTHETIC | `tools/multisuite_detector/build_detector_dataset_manifest_v1.py`; `detector_dataset_closure_v1.py` | `tests/multisuite_detector/test_detector_dataset_closure_v1.py` | C2 consumes `load_label_v2_artifact()`, exact-joins by `episode_key`, rejects set/parent/suite/task/trace mismatches, derives `DETECTOR_ELIGIBLE` and `DETECTOR_SAFETY`, and marks `DETECTOR_MULTI_EVENT` unavailable without a separate artifact. |
| Parent/state splits | EXISTS_AND_REVIEWED_AFTER_C2_SYNTHETIC | `tools/multisuite_detector/build_detector_splits_v1.py`; `detector_dataset_closure_v1.py` | `tests/multisuite_detector/test_detector_dataset_closure_v1.py` | C2 builds and validates parent-random, Object LOTO, and suite LOSO split manifests over connected components induced by `parent_key` and `initial_state_hash`, including state-hash leakage rejection. |
| Detector normalization manifest | EXISTS_AND_REVIEWED_AFTER_C2_SYNTHETIC | `tools/multisuite_detector/build_detector_normalization_v1.py`; `validate_detector_dataset_manifest_v1.py` | `tests/multisuite_detector/test_detector_dataset_closure_v1.py` | C2 computes train-only normalization for a selected split/fold/population, rejects zero variance, binds dataset/split/feature SHAs, and validates those bindings synthetically. Formal detector dataset build remains NOT_PERFORMED. |
