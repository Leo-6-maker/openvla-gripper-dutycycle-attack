#!/usr/bin/env python3
"""R7.1.2.1: K10 OPPORTUNITY LABELER — all gate/source/binding/attribution fixes.

Source: Physics Teacher V2.1 (SHA256SUMS 18f35203...)

Fixes over V1.2:
  1. segment_crossing & invalid_pos gates actually computed (no longer always-0)
  2. Per-row source validation on all 800 identities
  3. release/regrasp validity=None → non-positive (not treated as safe)
  4. Per-step component bitmask for accurate source attribution
  5. SOURCE_BINDING sealed with actual git commit + blob SHA
  6. MANIFEST, PROTOCOL, AUDIT all in single-pass seal
...
SCHEMA = "R7_K10_OPPORTUNITY_LABELER_V1_2_1"
        ...
        for task in range(N_TASKS):
            for state in FIT_STATES:
                ...
                for row_idx, r in enumerate(records):
                    total_rows += 1
                    if r.get("physics_protocol_schema") != "DETECTOR_V5_PHYSICS_TEACHER_PROTOCOL_V21":
...
def main():
    ...
    for suite, task, state in scope:
        ...
        with open(ident_out / "k10_labels_v121.jsonl", "w", encoding="utf-8") as fh:
    ...
    with open(out / "MANIFEST.json", "w", encoding="utf-8") as fh:
        json.dump({"schema": SCHEMA, "teacher_source": "Physics Teacher V2.1",
                    "teacher_sha256sums": TEACHER_SHA}, fh, indent=2)
