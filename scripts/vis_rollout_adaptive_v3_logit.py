...
from gripper_attack.uncertainty import extract_prefix_logits, softmax_np, gripper_entropy, prefix_top2_margin, prefix_entropy
...
def decode_image(img_np, instruction, return_gen=False):
...
while t < max_steps + num_steps_wait:
    ...
    if args.condition == 'clean' or not in_window:
        clean_logit_feats = {}
        raw_action, _, clean_gen_out = decode_image(img_np, cfg['instruction'], return_gen=True)
        try:
            logits_arr = extract_prefix_logits(clean_gen_out, action_dim)
            if logits_arr is not None and logits_arr.shape[0] > 0:
                gl = logits_arr[-1].astype(float).copy()
                gp = softmax_np(gl)
                mid = len(gp) // 2
                clean_logit_feats['gripper_logit_open_mass'] = float(np.sum(gp[:mid]))
                clean_logit_feats['gripper_logit_close_mass'] = float(np.sum(gp[mid:]))
                om = np.max(gl[:mid]) if mid > 0 else float('-inf')
                cm = np.max(gl[mid:]) if mid < len(gp) else float('-inf')
                olse = om + np.log(np.sum(np.exp(gl[:mid] - om))) if mid > 0 else float('-inf')
                clse = cm + np.log(np.sum(np.exp(gl[mid:] - cm))) if mid < len(gp) else float('-inf')
                clean_logit_feats['gripper_logit_margin'] = float(olse - clse) if np.isfinite(olse) and np.isfinite(clse) else 0.0
                clean_logit_feats['gripper_entropy'] = float(gripper_entropy(logits_arr))
                clean_logit_feats['gripper_top2_margin'] = float(prefix_top2_margin(logits_arr))
                clean_logit_feats['all_action_entropy'] = float(prefix_entropy(logits_arr))
        except Exception:
            pass
        ...
        clean_logit_feats = {}
        if ctrl_stopped:
            raw_action, _, _ctl_gen = decode_image(img_np, cfg['instruction'], return_gen=True)
            adv_grip = float(raw_action[-1]); clean_grip = adv_grip
            clean_action_vec = raw_action.copy()
        else:
            ...
            clean_logit_feats = {}
            try:
                clean_action, clean_token_ids, clean_gen_out = decode_image(img_np, cfg['instruction'], return_gen=True)
                clean_logit_feats = {}
                try:
                    logits_arr = extract_prefix_logits(clean_gen_out, action_dim)
                    if logits_arr is not None and logits_arr.shape[0] > 0:
    ...
    trace_rows.append({
        ...
        'gripper_logit_open_mass': clean_logit_feats.get('gripper_logit_open_mass', 0.0),
        'gripper_logit_close_mass': clean_logit_feats.get('gripper_logit_close_mass', 0.0),
        'gripper_logit_margin': clean_logit_feats.get('gripper_logit_margin', 0.0),
        'gripper_entropy': clean_logit_feats.get('gripper_entropy', 0.0),
        'gripper_top2_margin': clean_logit_feats.get('gripper_top2_margin', 0.0),
        'all_action_entropy': clean_logit_feats.get('all_action_entropy', 0.0),
