...
for w in range(3):
    ...
    for j in vis_jobs:
        ...
        lines.append('%s -u %s --task %s --state-id %s --window_start %s --window_end %s --condition vis_pgd --gpu_pair 0,1 --pgd_steps 20 --eps_raw_pixels 6 --max_steps 400 --job_id %s --output_dir %s; rc=$?; if [ $rc -ne 0 ]; then echo \"VIS_FAIL %s rc=$rc\"; fi' % (PY, SCRIPT, task, sid, ws, we, jid, OUT, jid))

        rj = rand_jobs.get(jid)
        if rj:
            rjid = rj['job_id']
            lines.append('echo "[$(date +%%H:%%M:%%S)] RAND %s: %s s%s [%s,%s]"' % (rjid, task, sid, ws, we))
            lines.append('%s -u %s --task %s --state-id %s --window_start %s --window_end %s --condition random_linf --gpu_pair 0,1 --eps_raw_pixels 6 --max_steps 400 --job_id %s --output_dir %s; rc=$?; if [ $rc -ne 0 ]; then echo \"RAND_FAIL %s rc=$rc\"; fi' % (PY, SCRIPT, task, sid, ws, we, rjid, OUT, rjid))
