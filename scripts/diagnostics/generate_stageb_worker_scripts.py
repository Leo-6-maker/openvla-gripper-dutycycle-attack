...
JOBS_CSV = sys.argv[1] if len(sys.argv) > 1 else '/data/liuyu/repos/openvla-gripper-dutycycle-attack-reviewed-20260605/tables/object100_next80_vis_label_jobs.csv'
...
for w in range(3):
    ...
    for j in vis_jobs:
        ...
        lines.append('%s -u %s --task %s --state-id %s --window_start %s --window_end %s --condition vis_pgd --gpu_pair 0,1 --pgd_steps 20 --eps_raw_pixels 6 --max_steps 400 --job_id %s --pair_id %s --output_dir %s; rc=$?; if [ $rc -ne 0 ]; then echo \"VIS_FAIL %s rc=$rc\"; fi' % (PY, SCRIPT, task, sid, ws, we, jid, pair_id, OUT, jid))

        rj = rand_jobs.get(jid)
        if rj:
            rjid = rj['job_id']
            lines.append('echo "[$(date +%%H:%%M:%%S)] RAND %s: %s s%s [%s,%s]"' % (rjid, task, sid, ws, we))
            lines.append('%s -u %s --task %s --state-id %s --window_start %s --window_end %s --condition random_linf --gpu_pair 0,1 --eps_raw_pixels 6 --max_steps 400 --job_id %s --pair_id %s --output_dir %s; rc=$?; if [ $rc -ne 0 ]; then echo \"RAND_FAIL %s rc=$rc\"; fi' % (PY, SCRIPT, task, sid, ws, we, rjid, pair_id, OUT, rjid))

    lines.append('')
    lines.append('echo "[$(date +%%H:%%M:%%S)] %s DONE"' % WORKER_NAME[w])

    tag = 'expansion' if '120' in JOBS_CSV else 'main'
    path = '/tmp/run_stageb_%s_%s.sh' % (WORKER_NAME[w], tag)
