...
def main():
    ...
    labels = []
    # Batch1
    if os.path.exists(args.batch1_merged):
        with open(args.batch1_merged, newline="") as f:
            for r in csv.DictReader(f):
                value, reason = classify_row(r)
                task = r.get("task",""); state = r.get("state_id","")
                ws = r.get("window_start",""); we = r.get("window_end","")
                ph = phase_map.get((task, state, ws, we), "")
                labels.append(dict(source="batch1", task_key=task, state_id=state,
                    window_start=ws, window_end=we, phase_bin_proxy=ph,
                    label_vulnerability_ready=value, label_reason=reason))

    # Batch2b
    if os.path.exists(args.batch2b_vis):
        with open(args.batch2b_vis, newline="") as f:
            for r in csv.DictReader(f):
                value, reason = classify_row(r)
                task = r.get("task",""); state = r.get("state_id","")
                ws = r.get("window_start",""); we = r.get("window_end","")
                ph = phase_map.get((task, state, ws, we), "")
                labels.append(dict(source="batch2b", task_key=task, state_id=state,
                    window_start=ws, window_end=we, phase_bin_proxy=ph,
                    label_vulnerability_ready=value, label_reason=reason))
