...
def run_one_attack(model, processor, img_np, instruction, clean_action, clean_gen,
                   ...
                   action_dim, low, seed, device):
    """Run TokenPrefixPGD on one frame, return metrics dict."""
    import torch
    t0 = time.time()
    error = ''
    result_row = {}
    try:
        ...
        clean_token = int(adv_decoded.token_ids[-1]) if hasattr(adv_decoded, 'token_ids') and adv_decoded.token_ids is not None else -1
        adv_token = int(adv_decoded.token_ids[-1]) if hasattr(adv_decoded, 'token_ids') and adv_decoded.token_ids is not None else -1
        # Actually get tokens from the raw decode result
        if hasattr(adv_decoded, 'token_ids') and adv_decoded.token_ids is not None:
            adv_tokens = adv_decoded.token_ids
        else:
            adv_tokens = np.zeros(action_dim, dtype=int)
        if hasattr(adv_decoded, 'clean_token_ids'):
            clean_tokens = adv_decoded.clean_token_ids
        else:
            clean_tokens = np.zeros(action_dim, dtype=int)

        clean_grip_token = int(clean_tokens[-1]) if len(clean_tokens) > 0 else -1
        adv_grip_token = int(adv_tokens[-1]) if len(adv_tokens) > 0 else -1
...
def load_frames_from_task(task_name, env_gpu=0):
    """Generate frames from LIBERO at known contact/carry/pre-place steps."""
    import torch
    ...
    # Collect frames at every step up to max_steps
    frames = []
    ...
    for step in range(300):
        ...
        # Step with zero action to stay in place (for frame collection only)
...
def main():
    ...
    for task_name in args.task:
        print(f'\n--- {task_name} ---')
        frames = load_frames_from_task(task_name, env_gpu=int(args.gpu_pair.split(',')[0]))
        # Sample frames: skip early (grasp setup) and sample evenly
                    ...
                    for steps in args.steps_list:
                        ...
                        for z_w in z_weights_iter:
                            ...
                            result = run_one_attack(
                                ...
                                clean_action, clean_gen, objective,
                                eps_proc, steps, step_size, z_w, grip_w,
