...
def is_pending_negative_1r(row):
    if has_bad_status(row):
        return False
    label_text = " ".join(lower(row.get(field)) for field in ["label_source", "label_confidence", "label_1r", "label_status", "inclusion_status", "status"])
    ...
    if open_count <= 1 and task_failed is False:
        return True
