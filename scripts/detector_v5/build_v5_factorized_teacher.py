#!/usr/bin/env python3
"""Build the FIT-only Factorized Teacher V1 derivative (Gate F2).

Narrow adapter of the mature build_v5_physics_teacher.py — keeps all
seal, registry, source verification, staging, and atomic-write infrastructure.
Only replaces: derive function, label filename, manifest/event schema.
"""
...
def _fit_rows(registry_csv: Path, target_split: str = "FIT_TRAIN") -> list[dict[str, Any]]:
    ...
    for row in fit:
        ...
        if row.get("canonical_parent_key") != expected or int(row["state_id"]) not in range(lo, hi + 1):
            raise ValueError(f"invalid {target_split} identity: {row.get('canonical_parent_key')}")
        if str(row.get("formal_selected", "")).lower() != "true":
            raise ValueError(f"{target_split} row not formal-selected: {row['canonical_parent_key']}")
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if protocol.get("schema") != FACTORIZED_TEACHER_SCHEMA:
        raise ValueError("unexpected Factorized Teacher protocol schema")
    v21c = False  # Factorized is its own lineage, not V21C
    ...
    try:
        ...
        for row in rows:
            ...
            _atomic_text(target / FACTORIZED_LABEL_FILENAME, "".join(json.dumps(item, sort_keys=True) + "\n" for item in derived))
            for event in events:
                event_rows.append({"canonical_parent_key": identity, **event})
            identity_count += 1
            step_count += len(derived)
            known_steps += sum(bool(item["grasp_established_known_mask"]) for item in derived)
            grasp_pos_steps += sum(1 for item in derived if item["grasp_established"])
            manip_pos_steps += sum(1 for item in derived if item["manipulation_active"])
            release_pos_steps += sum(1 for item in derived if item["release_or_instability"])
            route_counts.update(item["mechanism_type"] for item in derived)
        ...
        with (staging / "event_index.csv").open("w", newline="", encoding="utf-8") as handle:
            fields = ["canonical_parent_key", "event_id", "mechanism_type", "event_role",
                      "start_step", "end_step", "step_count", "has_grasp", "has_manipulation",
                      "has_release", "grasp_steps", "manipulation_steps", "release_steps",
                      "active_object_name"]
            writer = csv.DictWriter(handle, fieldnames=fields, extrasaction="ignore")
            writer.writeheader()
            writer.writerows(event_rows)
        manifest_schema = FACTORIZED_MANIFEST_SCHEMA
        teacher_version = "V1-FACTORIZED"
        manifest_filename = "factorized_teacher_v1_manifest.json"

        # Source bindings (always for factorized Teacher)
        action_contract_path = Path(_action_contract.__file__).resolve()
        action_contract_sha256 = sha256_file(action_contract_path)
        factorized_module = importlib.import_module("gripper_attack.v5_factorized_teacher")
        factorized_path = Path(factorized_module.__file__).resolve()
        if factorized_path.name != "v5_factorized_teacher.py":
            raise RuntimeError(f"factorized module resolved to unexpected file: {factorized_path}")
        factorized_sha256 = sha256_file(factorized_path)
        ...
        try:
            ...
            raise RuntimeError(f"git rev-parse HEAD failed in repo root: {repo_root}") from None
        if not re.fullmatch(r"[0-9a-f]{40}", git_commit):
            raise RuntimeError(f"git commit is not a valid 40-char hex SHA: {git_commit!r}")
        if args.expected_source_commit:
            if git_commit != args.expected_source_commit:
                raise RuntimeError(
                    f"source commit mismatch: expected {args.expected_source_commit} but HEAD is {git_commit}"
                )
        ...
        manifest = {
            ...
            "grasp_positive_steps": grasp_pos_steps,
            "manipulation_positive_steps": manip_pos_steps,
            "release_positive_steps": release_pos_steps,
            "event_count": len(event_rows),
            "task_count": 40,
            "task_role_status_counts": dict(sorted(role_counts.items())),
            "mechanism_route_counts": dict(sorted(route_counts.items())),
            "teacher_is_clean_only_physics_proxy": True,
            "counterfactual_attack_label": False,
            "student_future_leakage": False,
            "formal_training_authorized": False,
            "formal_attack_authorized": False,
            "action_contract_schema": _action_contract.ACTION_CONTRACT_SCHEMA,
            "action_contract_sha256": action_contract_sha256,
            "action_contract_source": str(action_contract_path),
