...
def run_one_episode(model, processor, device_ov, suite, task_index, task_name, state_id,
    ...
    for step_idx in range(max_steps):
        ...
        # Pre-step proprio — use EXACT same physical_gripper_state as Object collector
        from v4_run_eval_openvla import physical_gripper_state
        gs = physical_gripper_state(env, obs)
        qpos_ok = False
        if gs and gs.get("qpos") is not None and len(gs["qpos"]) > 0:
            qpos = float(np.sum(gs["qpos"]))
            qpos_ok = True
        else:
            qpos = float("nan")
        eef_ok = True
