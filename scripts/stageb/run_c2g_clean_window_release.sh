...
# Strict four-suite release orchestrator for C2g Detector-v2.
#
...
# - frozen CLEAN parent preservation;
# - strict matched-load execution and audit.
#
# `all` is executable but is not an authorization for expensive execution.
...
if [[ -n "$(git status --porcelain)" ]]; then
  echo "refusing C2g release pipeline from dirty worktree" >&2
  git status --short >&2
  exit 2
fi
...
export WORK_ROOT TRAIN_EPISODE_MANIFEST EVAL_PARENT_MANIFEST DEVICE WINDOW BURST_LENGTH
export EPOCHS BATCH_SIZE HIDDEN MAX_TRAIN_EPISODES MAX_EVAL_JOBS CONTROL_OBJECTIVE
export SUSCEPTIBILITY_POSITIVE_RETENTION SUITE_MODEL_MAP
