#!/usr/bin/env python3
"""Stage-B v1: Paired VIS PGD20 + random_linf runner.
Writes per-step trace with full action vector + correct qpos.
Usage: CUDA_VISIBLE_DEVICES=2,6 python -u scripts/stageb/run_paired_vis_random_v1.py \
    --gpu_pair 0,1 --task ketchup --state-id 0 --window_start 16 --window_end 33 \
    --condition vis_pgd --pair_id my_pair_0 --output_dir /path/to/out
"""
...
RUNNER_VERSION = 'patched_stageb_v1'
...
if _physical and _physical == [0, 1]: gpu_ids = [1, 0]; _render_gpu = 1
log('GPU: physical=%s render=%d runner=%s' % (_VISIBLE, _render_gpu, RUNNER_VERSION))
...
model = AutoModelForVision2Seq.from_pretrained(
    ...
    device_map='auto', max_memory={gpu_ids[0]: '10500MiB', gpu_ids[1]: '10500MiB', 'cpu': '64GiB'},
    trust_remote_code=True)
processor = AutoProcessor.from_pretrained(MODEL_PATH, trust_remote_code=True)
action_dim = int(model.get_action_dim(UNNORM_KEY))
model_device = next(model.parameters()).device; model_dtype = next(model.parameters()).dtype
...
def norm_invert(action):
    a = np.asarray(action, dtype=np.float32).copy()
    a[..., -1] = 2.0 * a[..., -1] - 1.0
    a[..., -1] = np.sign(a[..., -1])
    a[..., -1] = 1.0 if a[..., -1] == 0 else a[..., -1]
    a[..., -1] = -1.0 * a[..., -1]
    return a

_eps_eff = args.eps_raw_pixels / 255.0
attacker = TokenPrefixPGDAttacker(model=model, processor=processor, config={
...
}, seed=0, device='cuda:%d' % gpu_ids[0], preprocess_kwargs={'postprocess_gripper': True})
...
def make_inputs(pil_image, instruction):
    ...
    for k, v in list(inp.items()):
        if torch.is_floating_point(v): inp[k] = v.to(device=model_device, dtype=model_dtype)
        else: inp[k] = v.to(model_device)
    if not torch.all(inp['input_ids'][:, -1] == 29871):
        inp['input_ids'] = torch.cat((inp['input_ids'], torch.tensor([[29871]], dtype=torch.long, device=model_device)), dim=1)
    return inp

def model_generate(inp):
    with torch.inference_mode():
        gen = model.generate(**inp, max_new_tokens=action_dim, do_sample=False,
                             return_dict_in_generate=True, output_scores=False)
    return gen.sequences[0, -action_dim:].cpu().numpy()

TASK_CFG = {'ketchup':0,'butter':1,'cream_cheese':2,'salad_dressing':3,'bbq_sauce':4,
            'milk':5,'alphabet_soup':6,'tomato_sauce':7,'orange_juice':8}
...
log('%s [%d,%d] %s pair=%s' % (cid, ws, we, args.condition, args.pair_id))
...
try:
    ...
    if args.state_id >= len(initial_states): log('FATAL: state OOB'); sys.exit(1)
    ...
    log('INFRA: env init: %s' % str(e)[:80]); infra_status = 'env_init_fail'; env = None

if env is not None:
    current_step, done, qpos_before = 0, False, 0
    try:
        while not done and current_step < min(we + 5, args.max_steps):
            img = obs['agentview_image']; pil = Image.fromarray(img.astype(np.uint8))
            inputs = make_inputs(pil, instruction)
            clean_pv, clean_ids = inputs['pixel_values'], inputs['input_ids']

            # Qpos from obs (correct source)
            gq = obs.get('robot0_gripper_qpos', np.zeros(2))
            gripper_qpos_abs_mean = float((abs(gq[0]) + abs(gq[1])) / 2.0)

            # Decode clean action
            clean_action = decode_action(model_generate(inputs))
            clean_grip = norm_invert(clean_action.copy())[-1]

            in_window = 1 if ws <= current_step <= we else 0
            attack_this_step = in_window and args.condition != 'clean'
            env_grip, arm_l2 = clean_grip, 0.0
            pgd_applied, attacks_applied = 0, 0
            env_action_full = norm_invert(clean_action.copy())

            if attack_this_step:
                qpos_before = gripper_qpos_abs_mean
                if args.condition == 'vis_pgd':
                    try:
                        ...
                        adv_inp = get_adv_inputs_from_attack_result(result)
                        adv_pv = adv_inp['pixel_values'].to(device=model_device, dtype=model_dtype)
                        adv_ids = adv_inp['input_ids'].to(model_device)
                        adv_action = decode_action(model_generate({'input_ids': adv_ids, 'pixel_values': adv_pv}))
                        env_grip = norm_invert(adv_action.copy())[-1]
                        arm_l2 = float(np.linalg.norm((adv_action[:6] - clean_action[:6]).reshape(-1)))
                        pgd_applied, attacks_applied = 1, 1
                        env_action_full = norm_invert(adv_action.copy())
                    except Exception as e:
                        env_grip = clean_grip
                        infra_status = 'pgd_error'
                    ...
                    rand_action = decode_action(model_generate({'input_ids': clean_ids, 'pixel_values': rand_pv}))
                    env_grip = norm_invert(rand_action.copy())[-1]
                    arm_l2 = float(np.linalg.norm((rand_action[:6] - clean_action[:6]).reshape(-1)))
                    env_action_full = norm_invert(rand_action.copy())

            if in_window: decoded_grips.append(env_grip)
            ...
            trace_rows.append({
                ...
                'gripper_qpos': str(round(gripper_qpos_abs_mean, 6)),
                'done': str(int(done)),
            ...
            if in_window:  # qpos measured AFTER env.step
...
tag = '%s_%s_%s' % (args.task, args.condition, args.pair_id.replace(' ', '_'))

summary = {
    'pair_id': args.pair_id, 'task_key': args.task, 'state_id': str(args.state_id),
    'window_start': ws, 'window_end': we, 'condition': args.condition,
    'infra_status': infra_status, 'decoded_open_count': open_count,
    'decoded_longest_open_streak': max_streak, 'qpos_delta': round(total_qpos_delta, 6),
    'mean_arm_l2': round(mean_arm_l2, 6), 'n_window_steps': len(decoded_grips),
    'n_total_steps': current_step, 'runner_version': RUNNER_VERSION,
}
