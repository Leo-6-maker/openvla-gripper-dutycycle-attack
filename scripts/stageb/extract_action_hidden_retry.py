#!/usr/bin/env python3
"""S7 Action-hidden retry extraction — per-window checkpoint, append-only CSV."""
...
ap.add_argument('--output_dir', default='/data/liuyu/outputs/stageb_v1_1_k5c_targeted_expansion_rc1a_ca3a97e/action_hidden_full/retry_gpu26_shard45')
...
# Checkpoint: load already-completed window keys
...
if os.path.exists(COMPLETED):
    with open(COMPLETED) as f:
        for line in f:
            line = line.strip()
            if line: completed_keys.add(line)
    print('Resume: %d windows already completed' % len(completed_keys))

FAILED = os.path.join(args.output_dir, 'failed_window_ids.txt')

# Output CSV
CSV_OUT = os.path.join(args.output_dir, 'action_hidden_full_features_w%d_retry_gpu26.csv' % args.start)
...
for idx, (name, task_name, state_id, ws, we) in enumerate(unique):
    ...
    if window_key in completed_keys:
        print('[%d/%d] %s SKIP (already completed)' % (idx+1, len(unique), window_key))
        completed_count += 1
        continue
    ...
    try:
        ...
        # Per-window checkpoint: append + flush + fsync
        csv_exists = os.path.exists(CSV_OUT)
        ...
        completed_count += 1
        print('[%d/%d] %s pre=%d dim=%d OK' % (idx+1, len(unique), window_key, len(pre), hidden_dim))

    except Exception as e:
        failed_count += 1
        print('[%d/%d] %s FAILED: %s' % (idx+1, len(unique), window_key, str(e)[:120]))
        import traceback; traceback.print_exc()
        with open(FAILED, 'a') as f:
            f.write('%s | %s\n' % (window_key, str(e)[:200]))
            f.flush(); os.fsync(f.fileno())
