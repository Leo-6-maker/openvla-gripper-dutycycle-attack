#!/usr/bin/env python3
"""S20M1: Task-balanced RAND-only calibration queue for v0.3.2."""
import csv, json, glob, os, numpy as np
from collections import Counter

T='/data/liuyu/repos/codex_stageb_openvla_alignment_rc1a_20260607/tables'
O='/data/liuyu/outputs/stageb_s20m1_randonly_calibration_20260613'
os.makedirs(O+'/queues',exist_ok=True)

fresh_tasks={'alphabet_soup','bbq_sauce','butter','chocolate_pudding','cream_cheese','orange_juice','salad_dressing'}
...
clean_dirs=['/data/liuyu/outputs/stageb_s20k_clean_expansion_20260613',
...
for d in clean_dirs:
    for sf in sorted(glob.glob(d+'/summary_*clean*.json')):
        ...
        if task not in fresh_tasks:continue
...
for c in candidates:
    ...
    selected.append(c)
    task_n[c['task']]+=1;phase_n[c['phase']]+=1;adj_n[(c['task'],c['sid'])]+=1
...
for i,c in enumerate(selected):
    seed='90' if i%2==0 else '91'
    cid='%s_s%s_w%d_%d'%(c['task'],c['sid'],c['ws'],c['we'])
    jid+=1;jobs.append({'job_id':str(jid),'task':c['task'],'state_id':c['sid'],'window_start':str(c['ws']),'window_end':str(c['we']),'condition':'random_linf','attack_seed':seed,'random_control_seed':seed,'seed':'0','candidate_id':cid,'tier':'M1_'+c['tier'],'track':'S20M1','status':'pending'})
