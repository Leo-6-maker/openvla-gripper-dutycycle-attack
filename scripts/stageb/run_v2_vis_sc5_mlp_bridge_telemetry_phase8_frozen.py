...
bm = benchmark.get_benchmark_dict(); suite = bm[args.suite_name]()
...
env, obs = build_v4_exact_env(bddl, args.render_gpu, args.max_env_steps, 10)
obs = env.set_init_state(init_states[STATE_ID])
env, obs = apply_dummy_wait(env, obs, 10)

_task_name = task_obj.name

# Suite-aware object site lookup (registry-based, replaces hardcoded string parsing)
...
for step in range(args.max_env_steps):
    ...
    action, _, _, _ = decode_with_scores(model, processor, device, raw, instruction, args.unnorm_key, 8,
    ...
    if IS_ATTACK and _trigger_step >= 0 and step >= _trigger_step and attack_count < ATTACK_FRAMES:
        ...
        if IS_RAND:
            ...
            clean_gen.scores = []
            attack_result = attacker.attack(raw, instruction, clean_action_np, clean_action_np, clean_gen, unnorm_key=args.unnorm_key)
        ...
        s = model.get_action_stats(args.unnorm_key)
