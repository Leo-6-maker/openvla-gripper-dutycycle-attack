#!/usr/bin/env python3
"""Execute the strict matched-load matrix with exact per-suite OpenVLA models."""
...
STRICT = REPO / "scripts" / "stageb" / "run_c2g_matched_load_jobs_strict.py"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    total_launched = 0
    with tempfile.TemporaryDirectory(prefix="c2g_jobs_map_") as td:
        temporary = Path(td)
        for suite in SUITES:
            ...
            if suite == "libero_goal" and args.policy_model_manifest:
            ...
            if completed.returncode != 0:
                break
            total_launched += min(len(suite_jobs), remaining) if remaining > 0 else len(suite_jobs)

    status = (
        "PASS_C2G_MATCHED_LOAD_MODEL_MAP"
        if results and all(row["status"] == "PASS" for row in results)
        else "HOLD_C2G_MATCHED_LOAD_MODEL_MAP"
    )
    report = {
        "gate": "C2G_MATCHED_LOAD_MODEL_MAP",
        "status": status,
        "suite_model_map": str(args.suite_model_map.resolve()),
        "results": results,
    }
    args.output_root.mkdir(parents=True, exist_ok=True)
    (args.output_root / "c2g_matched_load_model_map_report.json").write_text(
