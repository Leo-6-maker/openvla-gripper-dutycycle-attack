#!/usr/bin/env python3
"""V6 batch clean observer: loads model once, runs multiple task/state pairs."""
import os, sys, time

GPU_PAIR = sys.argv[1]  # "0,1"
RENDER = int(GPU_PAIR.split(',')[0])
OUT = sys.argv[2]

# task,state_id pairs from remaining args
pairs = []
...
for rep in range(2):  # 2 reps per parent
    for task, state_id in pairs:
        ...
        cmd = [PY, '-u', RUNNER,
            ...
            '--model_path', MODEL,
            '--render_gpu_device_id', str(RENDER), '--model_gpu_device_id', '-1',
            '--output_dir', OUT, '--job_id', 'v6b_%s' % tag,
...
print('[%s] All done' % time.strftime('%H:%M:%S'), flush=True)
