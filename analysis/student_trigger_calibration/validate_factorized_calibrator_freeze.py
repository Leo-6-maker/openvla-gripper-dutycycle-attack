#!/usr/bin/env python3
"""Validate a FACTORIZED_CALIBRATOR_FREEZE_V1.json contract.

FAIL-CLOSED: validates schema, seal, identity role=C, no H/A binding,
all heads frozen with known pos/neg, no fallback, source SHA closure.
"""
...
def main() -> int:
    ...
    ap.add_argument("--freeze-contract-root", type=Path, required=True,
                    help="Sealed output directory from freeze_factorized_calibrators.py")
    ...
    contract_path = freeze_root / "FACTORIZED_CALIBRATOR_FREEZE_V1.json"
    contract = load_strict_json(contract_path, "FREEZE_CONTRACT")

    errors: list[str] = []

    # Schema check
    if contract.get("schema") != "FACTORIZED_CALIBRATOR_FREEZE_V1":
    ...
    for key in ("phase_b_receipt_sha256", "cp_prediction_validation_receipt_sha256",
    ...
    if n_splits != 12:
        errors.append(f"SPLIT_COUNT: expected 12 got {n_splits}")

    frozen_heads_count = 0
    hold_heads: list[str] = []
    for sk in sorted(per_split):
        split_data = per_split[sk]
        for head in HEADS:
            hd = split_data.get(head, {})
            if not isinstance(hd, dict):
                errors.append(f"HEAD_MISSING: {sk}/{head}")
                continue
            method = hd.get("method", "")
            if method not in ("RAW", "INTERCEPT_ONLY", "PLATT"):
                errors.append(f"METHOD_INVALID: {sk}/{head} method={method}")
            if not hd.get("method_valid"):
                hold_heads.append(f"{sk}/{head}")
                continue
            n_pos = hd.get("n_fit_pos", 0)
            n_neg = hd.get("n_fit_neg", 0)
            if n_pos == 0:
                hold_heads.append(f"{sk}/{head}: NO_POSITIVE")
            elif n_neg == 0:
                hold_heads.append(f"{sk}/{head}: NO_NEGATIVE")
            else:
                frozen_heads_count += 1
            a_val = hd.get("a")
            b_val = hd.get("b")
            if isinstance(a_val, bool) or not isinstance(a_val, (int, float)) or not math.isfinite(float(a_val)):
                errors.append(f"PARAM_A_INVALID: {sk}/{head}")
            if isinstance(b_val, bool) or not isinstance(b_val, (int, float)) or not math.isfinite(float(b_val)):
                errors.append(f"PARAM_B_INVALID: {sk}/{head}")

    # At least one head must be frozen overall
    if frozen_heads_count == 0:
        errors.append("NO_HEADS_FROZEN")

    staging = out_root.with_name(f".{out_root.name}.{uuid.uuid4().hex}.staging")
    staging.mkdir(parents=True)

    validation = {
        "schema": "FACTORIZED_CALIBRATOR_FREEZE_VALIDATION_V1",
        "validator_code_sha256": SELF_SHA,
        "status": "PASS" if not errors else "HOLD",
        "errors": errors,
        "hold_heads": hold_heads,
        "frozen_heads_count": frozen_heads_count,
        "total_heads": n_splits * 3,
        "freeze_contract_sha256": sha256_file(contract_path),
        "attack_authorized": False,
        "heldout_l3_authorized": False,
        "mode": args.mode,
    }
    (staging / "FACTORIZED_CALIBRATOR_FREEZE_VALIDATION_V1.json").write_text(
    ...
    print(f"Calibrator Freeze Validation: {validation['status']}")
    for e in errors:
        print(f"  ERROR: {e}")
    for h in hold_heads:
        print(f"  HOLD: {h}")
