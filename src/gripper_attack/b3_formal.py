...
def validate_training_authorization(value: Mapping[str, Any]) -> None:
    required = (
        ...
        *AUTHORIZATION_INPUT_NAMES, "runner_head", "runner_binding", "authorization_generation",
        ...
        "normalization_file_sha256", "policy_intent_root_sha256",
    )
    ...
    if not isinstance(value.get("normalization_file_sha256"), str) or not _is_sha(value["normalization_file_sha256"]):
        raise ValueError("authorization normalization file SHA is missing")
    policy_root_sha = value.get("policy_intent_root_sha256")
    consumed_9d = value.get("verification", {}).get("policy_intent_root_consumed") is True
    if value.get("variant") == "B3_25D":
        if policy_root_sha is not None or consumed_9d:
            raise ValueError("B3_25D authorization cannot consume a 9D root")
    elif not _is_sha(policy_root_sha) or not consumed_9d:
        raise ValueError("B3_25D9D authorization must bind a sealed 9D root")
